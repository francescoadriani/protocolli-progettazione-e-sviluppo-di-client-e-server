...
namespace RandomizzatoreServerPool
{
    ...
    public class Server
    {
        ...
        public void start()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    ClientEPAndMessage sc = new ClientEPAndMessage(byteData, bytesReceived, clientEP);
                    sc.AnswerReadyEvent += Sc_AnswerReadyEvent;
                    ThreadPool.QueueUserWorkItem(MessageHandler.handle, sc);
                }
            }
        ...
        private void Sc_AnswerReadyEvent(object sender, AnswerReadyEventArgs e)
        {
            server.SendTo(e.answer, e.clientEP);
        }
    }
}
